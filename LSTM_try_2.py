...
def calculate_indicators(df):
    ...
    try:
        ...
        stock_data['Upper_Band'] = stock_data['MA_20'] + (stock_data['std_dev'] * 2)
        stock_data['Lower_Band'] = stock_data['MA_20'] - (stock_data['std_dev'] * 2)
...
def get_stock_data(ticker, days):
    ...
    try:
        ...
        # Download stock data
        stock_data = yf.download(ticker, start=start_date, end=end_date, progress=False, multi_level_index=False)
...
def custom_loss(y_true, y_pred):
    # Convert y_true to one-hot encoding since the model outputs softmax probabilities
    y_true_one_hot = tf.one_hot(tf.cast(y_true, tf.int32), depth=3) + 1 # Shift labels from [-1,0,1] to [0,1,2]
    
    # Calculate MSE between true and predicted probabilities
    mse = tf.reduce_mean(tf.square(y_true_one_hot - y_pred))
    
    # Normalize MSE to [-1, 1] range
    max_mse = 10  # Adjust based on your dataset
    min_mse = 0
    scaled_loss = (mse - min_mse) / (max_mse - min_mse) * 2 - 1
    return scaled_loss
...
def generate_trading_signals_with_ml(stock_data):
    ...
    try:
        ...
        # Create a target column (1 for Buy, -1 for Sell, 0 for Hold)
        ...
        df['target'] = df['target'].fillna(0)  # Default to Hold
        
        # Normalize the input features
        scaler = StandardScaler()
        X = scaler.fit_transform(df[['MACD', 'RSI', 'Close', 'Upper_Band', 'Lower_Band']])
        y = df['target'].values
        ...
        model = Sequential([
            ...
            Dropout(0.2),
            Dense(1, activation='tanh')  # Output 3 classes (-1, 0, 1)
        ])
        ...
        # y_pred = np.argmax(y_pred, axis=1) - 1  # Convert from [0,1,2] to [-1,0,1]
        ...
        # Predict signals on the full dataset
        signals = model.predict(X)
        signals = np.argmax(signals, axis=1)
        ...
        dbc.Col([
            dbc.Card([
                dbc.CardBody([
                    ...
                    html.Label("Initial Balance INR"),
                    dcc.Input(
                        id='balance-input',
                        value=10000,
                        type='number',
...
def update_dashboard(n_clicks, nifty_clicks, ml_clicks, ticker, days, initial_balance):
